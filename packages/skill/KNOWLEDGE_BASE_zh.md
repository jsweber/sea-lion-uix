...
传入 `ossUrl` 时，组件按链接类型自动判断：`.js` 为 Symbol 多色，`.css` 为 Font-class 单色；此时 `useSymbol` 不参与判断。
...
- **内置链接**：不传 `ossUrl` 时使用 `defaultOssIconCssUrl`（Font-class）或 `defaultOssIconJsUrl`（Symbol），由 **useSymbol** 切换，默认 `false`。
- **Font-class**：单色，可设置 `color`、`fontSize`。
- **Symbol**：多色，组件内部用 SVG `<use>` 渲染；用法 `<IconFont icon="icon-xxx" useSymbol />`。
...
| ossUrl     | string    | 见上文  | 资源地址；不传时用内置链接，由 useSymbol 决定 |
| useSymbol  | boolean   | false   | 是否用 Symbol 多色；仅在不传 ossUrl 时生效 |
| color      | string    | currentColor | 图标颜色（Font-class） |
| fontSize   | string    | 1em     | 图标尺寸 |
| style / className | —    | —       | 样式 |

导出的常量：`defaultOssIconCssUrl`、`defaultOssIconJsUrl`、`defaultOssIconUrl`（已废弃）、`isSymbolOssUrl(url)`。

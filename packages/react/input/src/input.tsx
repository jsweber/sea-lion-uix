...
import { inputRootPropDefs, inputFieldSlotPropDefs } from './input.props';
import { extractProps } from '@sea-lion/react-helpers';
import { marginPropDefs } from '@sea-lion/react-props';

import type { ComponentPropsWithout, RemovedProps, NotInputTextualAttributes } from '@sea-lion/react-helpers';
import type { MarginProps, GetPropDefTypes } from '@sea-lion/react-props';

type TextFieldRootElement = React.ElementRef<'input'>;
type TextFieldRootOwnProps = GetPropDefTypes<typeof inputRootPropDefs> & {
...
type TextFieldSlotOwnProps = GetPropDefTypes<typeof inputFieldSlotPropDefs>;
...
const TextFieldSlot = React.forwardRef<TextFieldSlotElement, TextFieldSlotProps>(
    (props, forwardedRef) => {
        const { className, color, side, ...slotProps } = extractProps(props, inputFieldSlotPropDefs);
        return (
            <div
                data-accent-color={color}
                data-side={side}
